...
def plot_nice1D(channel, var, bin, low, high, ylabel, xlabel, save, setLog = False, finalcuts = False):
    ...
    List = [ 'DiPhotonJets','DiphotonBox_Pt_25to250','DiphotonBox_Pt_250toInf','WtoLNuMu','WtoLNuTau','WGamma','ZGToLLG','ZGamma_Inclusive','efake_dd','qcd_dd','PJets_15to30','PJets_30to50','PJets_50to80','PJets_80to120','PJets_120to170','PJets_170to300','PJets_300to470', 'PJets_470to800', 'PJets_800to1400', 'PJets_1400to1800','PJets_1800','SinglePhotonParked','DM','MChi_70']
    ...
    for Type in List:
        ...
        #SUSYHIGGS
        #cut_standard= "(Pho_Pt > 45 && Pho_Pt<60.  && Pho_R9>0.9 &&TMath::Abs(Pho_Eta)<1.442 && sigmaIEtaIEta > 0.001  && sqrt(sigmaIPhiIPhi) > 0.001  && seedCrystalEnergy/(e1x3+e1x5-seedCrystalEnergy) < 0.9 &&  Pho_R9<1.0  && MET>40. && MetSig > 20 && loose_mu_10==0 && foundvetoEl_10 == 0 && MT > 100. && minMET>50.)" 
...
#        if Type.startswith("GJets_"):
            ...
            Trees[Type].Draw(var + " >> " + histName,  "(" + cut_standard +  ") * 1.6 * "+weight   , "goff")
            Variables[Type].Scale(lumi / Nevents[Type] * xsec[Type])
            stack.Add(Variables[Type],"hist")
            added.Add(Variables[Type])
            added_mconly.Add(Variables[Type])            
...
#        if Type.startswith("GJets_"):
    ...
    List_1 = [ 'DiPhotonJets','DiphotonBox_Pt_25to250','DiphotonBox_Pt_250toInf','WtoLNuMu','WtoLNuTau','WGamma','ZGToLLG','ZGamma_Inclusive','efake_dd','qcd_dd','PJets_15to30','PJets_30to50','PJets_50to80','PJets_80to120','PJets_120to170','PJets_170to300','PJets_300to470', 'PJets_470to800', 'PJets_800to1400', 'PJets_1400to1800','PJets_1800']
...
#var = 'Pho_Pt'; bin = 15; low = 45; high = 60; xaxis = "Photon P_{T} [GeV]"; yaxis = 'Events / 25 GeV'; save = 'Pho_Pt'; setLog = True; finalcuts = False;channel = "VBF";
