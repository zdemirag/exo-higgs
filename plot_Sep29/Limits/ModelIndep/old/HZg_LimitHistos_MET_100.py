...
def makeHistos(channel,var, bin, xlow, finalcuts,varname):
    ...
    for Type in List:
        ...
        cut_standard ="(Pho_Pt > 45 && Pho_R9>0.9 &&TMath::Abs(Pho_Eta)<1.442 && sigmaIEtaIEta > 0.001  && sqrt(sigmaIPhiIPhi) > 0.001  &&  Pho_R9<1.0    && seedCrystalEnergy/(e1x3+e1x5-seedCrystalEnergy) < 0.9 && MET>"+str(100)+" && MT > "+str(MTCUT)+" && loose_mu_10==0 && foundvetoEl_10==0 && n_jets_mva_loose < 2 && (TMath::Abs(dphi_jet_pho) <2.5 || dphi_jet_pho == -99) )"
    ...
    binname = 'MET'+str(100)+'_MT'+str(MTCUT)
    datacard = open('datacard_'+binname+'.txt', 'w')
    datacard.write( "#  datacard for model independent MET >"+str(100)+" \n")
