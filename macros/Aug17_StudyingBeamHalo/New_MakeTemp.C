...
Bool_t New_MakeTemp::Process(Long64_t entry)
{
    ...
    //if (!(Pho_SigmaIEtaIEta<0.015) ) return kTRUE;
    //if( !(TMath::Sqrt(sigmaIPhiIPhi)>0.0087) ) return kTRUE;
    ...
    if(passedDen==1 && Pho_SigmaIEtaIEta < 0.015){
...
void New_MakeTemp::GetAllEntries(Long64_t entry)
{
   ...
   //std::cout << "added pho pt "<<std::endl;
   b_Pho_SigmaIEtaIEta->GetEntry(entry);
   //std::cout << "added ieta "<<std::endl;
   b_sigmaIPhiIPhi->GetEntry(entry);
   //std::cout << "added iphi "<<std::endl;
...
//   std::cout << "finished entries" << std::endl;
}
