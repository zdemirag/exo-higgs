...
void phiCorr::Loop()
{
  ...
  if (fChain == 0) return;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      // 40To100
      //float MEx_corr = MEx_old - (0.2017 - 0.0341773*NVtx);
      //float MEy_corr = MEy_old - (-0.0116271 -0.101238*NVtx);

      //100To200
      //float MEx_corr = MEx_old - (0.21531 - 0.0318697*NVtx);
      //float MEy_corr = MEy_old - (-0.226865 - 0.0939224*NVtx);

      //200To400
      //float MEx_corr = MEx_old - (0.353619 - 0.0304403*NVtx);
      //float MEy_corr = MEy_old - (-0.224315 - 0.0913838*NVtx);

      //400
      float MEx_corr = MEx_old - (0.554642 - 0.0342045*NVtx);
      float MEy_corr = MEy_old - (-0.274867 - 0.076623*NVtx);
