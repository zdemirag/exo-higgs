...
def makeHistos(channel,var, bin, xlow, finalcuts,varname):
    ...
    for Type in List:
        ...
        cut_standard= "(Pho_Pt > 45 && Pho_Pt<60.  && Pho_R9>0.9 &&TMath::Abs(Pho_Eta)<1.442 && sigmaIEtaIEta > 0.001  && sqrt(sigmaIPhiIPhi) > 0.001  && seedCrystalEnergy/(e1x3+e1x5-seedCrystalEnergy) < 0.9 &&  Pho_R9<1.0  && MET>40. && MetSig > 20 && loose_mu_10==0 && foundvetoEl_10 == 0 && MT > 100. && minMET>45. && TMath::Abs(TMath::Log10(pvalue)) > 2 && angle>1.2 && HT < 100)"
    ...
    Variables['WtoLNuTau'].Add(Variables['ZGToLLG'])
    Variables['WtoLNuTau'].Add(Variables['DiphotonBox_Pt_250toInf'])
    Variables['WtoLNuTau'].Add(Variables['DiphotonBox_Pt_25to250'])
    Variables['WtoLNuTau'].Add(Variables['DiPhotonJets'])
    Variables['WtoLNuTau'].SetName('Other')
    Variables['WtoLNuTau'].SetTitle('Other')
    ...
    datacard.write( '--------------- \n')
    datacard.write( 'bin     '+binname+' '+binname+' '+binname+' '+binname+' '+binname+' '+binname+' '+binname+' \n')
    datacard.write( 'process '+binname+'  Other  WGamma  ZGamma    QCD     EFake   PJets \n')
    ...
    datacard.write( 'rate    '+str(round(Variables['SUFFIX'].Integral(),3))+'     '+str(round(Variables['WtoLNuTau'].Integral(),3))+' '+str(round(Variables['WGamma'].Integral(),3))+' '+str(round(Variables['ZGamma_Inclusive'].Integral(),3))+' '+str(round(Variables['qcd_dd'].Integral(),3))+' '+str(round(Variables['efake_dd'].Integral(),3))+' '+str(round(Variables['GJets_HT_40To100'].Integral(),3))+' \n')
    datacard.write( '--------------- \n')
    datacard.write( 'lumi    lnN 1.026 1.026 1.026 1.026 1.0   1.0   1.026 \n')
    datacard.write( 'qcd     lnN -            -     -    -    1.35   -     -    \n')
    datacard.write( 'efake   lnN -            -     -    -     -    1.06   -    \n') 
    datacard.write( 'phoEn   lnN 1.04    1.04  1.04  1.04  -   -     1.04    \n')
    datacard.write( 'metsys  lnN 1.04    1.04  1.04  1.04  -   -     1.04    \n')
    datacard.write( 'kfacorP lnN -          -  -  -      -     -    1.16  \n')
    datacard.write( 'kfacorW lnN -          1.05  -  -      -     -    -  \n')
    datacard.write( 'kfacorZ lnN -          -  1.05  -      -     -    -  \n')
    datacard.write( 'pusys   lnN 1.01     1.01  1.01  1.01      -     -    1.01  \n')
